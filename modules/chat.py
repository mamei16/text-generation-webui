...
def generate_chat_prompt(user_input, state, **kwargs):
    ...
    def make_prompt(messages):
        ...
        if (state['mode'] == 'instruct' and 'enable_thinking' in state['instruction_template_str']
                and not any((_continue, impersonate, state['enable_thinking']))):
...
def handle_remove_last_click(state):
    ...
    return [history, {"text": last_input, "files": []}]
...
def handle_branch_chat_click(state):
    ...
    return [history, past_chats_update, -1]
