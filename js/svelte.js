	...
	do {
		...
		try {
			while (flushidx < dirty_components.length) {
				...
				if (typeof(currentlyGenerating) !== "undefined" && currentlyGenerating && skipUIUpdatesDuringStreaming) {
					component.$$.dirty = [-1];
					continue;
				}
				update(component.$$);
...
function update($$) {
	if ($$.fragment !== null) {
		...
		const dirty = $$.dirty;
		$$.dirty = [-1];
		$$.fragment && $$.fragment.p($$.ctx, dirty);  // !currentlyGenerating && TODO: `p` is the culprit
